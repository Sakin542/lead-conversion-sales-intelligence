...
export const Footer: React.FC<FooterProps> = ({ className = '' }) => {
  ...
  const footerBgClass = 'bg-[#0A0A0A] border-t border-[#222222] text-zinc-400';
    ...
    setActiveModal({
      ...
      content: (
        <div className="space-y-4 text-xs sm:text-sm text-zinc-300 leading-relaxed">
          ...
          <h4 className="font-semibold text-white text-sm">1. Data Collection</h4>
          <p>
          ...
          <h4 className="font-semibold text-white text-sm">2. Data Security & Encryption</h4>
          <p>
          ...
          <h4 className="font-semibold text-white text-sm">3. Compliance</h4>
          <p>
    ...
    setActiveModal({
      ...
      content: (
        <div className="space-y-4 text-xs sm:text-sm text-zinc-300 leading-relaxed">
          ...
          <h4 className="font-semibold text-white text-sm">1. Subscription & Account Access</h4>
          <p>
            Accounts are provisioned for authorized enterprise revenue personnel. Users must maintain credential confidentiality.
          </p>
          <h4 className="font-semibold text-white text-sm">2. Permitted Use</h4>
          <p>
          ...
          <h4 className="font-semibold text-white text-sm">3. Service Level Agreement (SLA)</h4>
          <p>
  ...
  const openSecurityModal = () => {
    setActiveModal({
      ...
      content: (
        <div className="space-y-4 text-xs sm:text-sm text-zinc-300 leading-relaxed">
          <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center space-x-3 text-emerald-400 font-medium">
            <ShieldCheck className="w-5 h-5 shrink-0" />
            <span>SOC-2 Type II Certified & ISO 27001 Compliant</span>
          </div>
          <h4 className="font-semibold text-white text-sm">Enterprise Infrastructure Protection</h4>
  ...
  const openSolutionModal = (solutionTitle: string, description: string, benefits: string[]) => {
    setActiveModal({
      ...
      content: (
        <div className="space-y-4 text-xs sm:text-sm text-zinc-300 leading-relaxed">
          <p>{description}</p>
          <h4 className="font-semibold text-white text-sm">Key Capabilities & Benefits</h4>
          <ul className="list-disc pl-5 space-y-1.5 text-zinc-400">
  ...
  return (
    <footer className={`${footerBgClass} relative overflow-hidden transition-colors duration-300 ${className}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="md:col-span-5 space-y-6">
            ...
            <div className="space-y-2 max-w-sm">
              ...
              {newsletterSubscribed ? (
                <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center space-x-2.5 text-emerald-400 text-xs font-medium">
                ...
                <form onSubmit={handleNewsletterSubmit} className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full bg-[#050505] border border-[#222222] text-white text-xs rounded-lg pl-9 pr-3 py-2.5 focus:outline-none focus:border-violet-500 transition-colors placeholder-zinc-500"
                    />
                  </div>
              ...
              {newsletterError && <p className="text-xs text-rose-400 font-medium">{newsletterError}</p>}
            </div>
          </div>
          ...
          <div className="md:col-span-2 space-y-3">
            ...
            <ul className="space-y-2 text-sm text-zinc-400">
              <li>
                <button onClick={() => handleNavClick('#demo')} className="hover:text-white transition-colors text-left">
                  AI Lead Simulator
                </button>
              </li>
              <li>
                <button onClick={() => handleNavClick('#features')} className="hover:text-white transition-colors text-left">
                  Feature Suite
                </button>
              </li>
              <li>
                <button onClick={() => handleNavClick('#how-it-works')} className="hover:text-white transition-colors text-left">
                  Workflow Guide
                </button>
              </li>
              <li>
          ...
          <div className="md:col-span-3 space-y-3">
            ...
            <div className="space-y-2 pt-1">
              <Link to="/login" className="block text-sm text-zinc-300 hover:text-white transition-colors">
                Sign In to CRM Dashboard
              </Link>
            </div>

            <div className="pt-4 space-y-2 border-t border-[#222222] text-xs">
              <button onClick={openSecurityModal} className="flex items-center space-x-2 text-zinc-400 hover:text-white transition-colors">
        ...
        {/* Bottom Bar */}
        <div className="mt-12 pt-8 border-t border-[#222222] flex flex-col sm:flex-row items-center justify-between text-xs text-zinc-500 gap-4">
          <p>© 2026 Predictive Lead Scoring & CRM. All rights reserved.</p>
          <div className="flex space-x-6">
            <button onClick={openPrivacyModal} className="hover:text-zinc-300 transition-colors">
              Privacy Policy
            </button>
            <button onClick={openTermsModal} className="hover:text-zinc-300 transition-colors">
              Terms of Service
            </button>
            <button onClick={openSecurityModal} className="hover:text-zinc-300 transition-colors">
              Security
            </button>
          </div>
        </div>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
          <div className="bg-[#111111] border border-[#222222] rounded-xl max-w-lg w-full p-6 space-y-5 shadow-2xl relative text-white">
            <div className="flex items-center justify-between border-b border-[#222222] pb-3">
              <div className="flex items-center space-x-2.5">
                {activeModal.icon}
                <h3 className="text-base font-semibold text-white">{activeModal.title}</h3>
              </div>
              <button
