        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              <Card className="p-4 bg-slate-900/90 border-slate-800 flex flex-col justify-between h-full space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">My Leads</span>
                  <Users className="w-5 h-5 text-indigo-400" />
                </div>
                <div>
                  <p className="text-2xl font-black text-white">{kpis.my_leads || 0}</p>
                  <p className="text-[11px] text-slate-500 mt-0.5">Assigned Accounts</p>
                </div>
              </Card>
              ...
              <Card className="p-4 bg-slate-900/90 border-slate-800 flex flex-col justify-between h-full space-y-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 mt-0.5">Score &ge; 80 (High Intent)</p>
                </div>
              </Card>

              <Card className="p-4 bg-slate-900/90 border-slate-800 flex flex-col justify-between h-full space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Follow-ups Today</span>
                  <Clock className="w-5 h-5 text-cyan-400" />
                </div>
                <div>
                  <p className="text-2xl font-black text-cyan-400">{kpis.followups_today || 0}</p>
                  <p className="text-[11px] text-slate-500 mt-0.5">Scheduled Actions</p>
                </div>
              </Card>
              ...
              <Card className="p-4 bg-slate-900/90 border-slate-800 flex flex-col justify-between h-full space-y-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 mt-0.5">Lead to Deal Ratio</p>
                </div>
              </Card>

              <Card className="p-4 bg-slate-900/90 border-slate-800 flex flex-col justify-between h-full space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Deals Won</span>
                  <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                </div>
                <div>
                  <p className="text-2xl font-black text-emerald-400">{kpis.deals_won || 0}</p>
                  <p className="text-[11px] text-slate-500 mt-0.5">Closed Wins</p>
                </div>
              </Card>

              <Card className="p-4 bg-slate-900/90 border-slate-800 flex flex-col justify-between h-full space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Revenue Won</span>
                  <DollarSign className="w-5 h-5 text-emerald-400" />
                </div>
                <div>
                  <p className="text-2xl font-black text-emerald-400">${(kpis.revenue || 0).toLocaleString()}</p>
                  <p className="text-[11px] text-slate-500 mt-0.5">Closed Sales Value</p>
                </div>
              </Card>
              ...
              <Card className="p-4 bg-slate-900/90 border-slate-800 flex flex-col justify-between h-full space-y-2">
                ...
                <div>
                  <p className="text-2xl font-black text-purple-400">${(kpis.pipeline_value || 0).toLocaleString()}</p>
                  <p className="text-[11px] text-slate-500 mt-0.5">Active Deals Funnel</p>
                </div>
              </Card>

              <Card className="p-4 bg-slate-900/90 border-slate-800 flex flex-col justify-between h-full space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Active Model</span>
                  <Sparkles className="w-5 h-5 text-indigo-400" />
                </div>
                <div>
                  <p className="text-sm font-extrabold text-white truncate">{kpis.active_model || 'XGBoost'}</p>
                  <p className="text-[11px] text-slate-500 mt-0.5">AI Scoring Engine</p>
                </div>
              </Card>
            </div>
              ...
              <Card className="p-5 bg-slate-900/80 border-slate-800 flex flex-col justify-between h-full space-y-4">
                <div className="space-y-4">
                  ...
                  {priorities.hot_leads_no_contact?.length === 0 ? (
                    <p className="text-xs text-slate-500 py-6 text-center">No uncontacted HOT leads right now.</p>
                  ) : (
                        ...
                        <div key={l.id} className="p-3.5 bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Button
                              ...
                              className="text-xs border-indigo-800 text-indigo-300"
                            >
              ...
              <Card className="p-5 bg-slate-900/80 border-slate-800 flex flex-col justify-between h-full space-y-4">
                <div className="space-y-4">
                  ...
                  {priorities.overdue_followups?.length === 0 ? (
                    <p className="text-xs text-slate-500 py-6 text-center">All follow-ups are up to date! Good job.</p>
                  ) : (
                    <div className="space-y-3">
